...
var busLocations = {
  ...
  "E Stevens WAY NE and Jefferson RD NE": [47.654503, -122.30513],
  "Stevens Way and Asotin Pl": [47.654377, -122.310486],
  "NE Pacific Pl and NE Pacific St": [47.650505, -122.306908],
  "NE Pacific Pl and Montlake Blvd NE - Bay 1": [47.649143, -122.30497],
...
z =
  "https://www.washington.edu/maps/wp-content/themes/maps-2014/tiles/retina/{z}/{x}/{y}.png";
...
// mapMarker
var singleMarker = L.marker([6.6904378395789985, -1.6149902498988071]);
var popup = singleMarker.bindPopup("AAMUSTED").openPopup()
popup.addTo(mymap);
...
fetch("https://hnavcontent.azurewebsites.net/PublishedNodes.txt")
  ...
  .then((data) => {
    ...
    populateNavOptions(
      [buildingLocations, libraryLocations, busLocations],
      startSelection
    );
    populateNavOptions(
      [buildingLocations, libraryLocations, busLocations],
...
var feedbackHeader=document.getElementById("feedbackHeader");
...
var buildingsHeader = document.getElementById("buildingsHeader");
var librariesHeader = document.getElementById("librariesHeader");
...
document
  .getElementById("startingPointsId")
  .addEventListener("change", updateStart);
document
  .getElementById("destinationsId")
  .addEventListener("change", updateDest);
...
function toggleTheme() {
  if (this.checked) {
    // Represents dark mode.
    darkMode();
  } else {
    // Represents light mode.
    lightMode();
  }
}
...
function darkMode() {
  ...
  feedbackHeader.style.color="#006938";
  stopHeader.style.color="#006938";
  buildingsHeader.style.color="#006938";
  navHeader.style.color="#006938";
  aboutHeader.style.color="#006938";
  librariesHeader.style.color="#006938";
  fromElement.style.color="#006938";
  toElement.style.color="#006938";
  distanceElement.style.color="#006938";
  etaElement.style.color="#006938";
  horizontalBar.style.backgroundColor = "#FFFFFF";
  leftSideBar.style.backgroundColor = "#FFFFFF";
  titleElements.style.backgroundColor = "#8C003B";
  weatherArrow.style.fill = "#202225";
  weatherPopupBody.style.backgroundColor = "#202225";
  wordmark.src = "./src/assets/uwWordmarks/UnivWaWordmark.png";
  title.style.color = "whitesmoke";
  logo.src = "./src/assets/logos/HuskyNavLogoDarker.png";
...
function lightMode() {
  ...
  title.style.color = "#006938";
  buildingsHeader.style.color="#FFFFFF";
  feedbackHeader.style.color="#FFFFFF";
  stopHeader.style.color="#FFFFFF";
  buildingsHeader.style.color="#FFFFFF";
  navHeader.style.color="#FFFFFF";
  aboutHeader.style.color="#FFFFFF";
  librariesHeader.style.color="#FFFFFF";
  fromElement.style.color="#FFFFFF";
  toElement.style.color="#FFFFFF";
  distanceElement.style.color="#FFFFFF";
  etaElement.style.color="#FFFFFF";


  horizontalBar.style.backgroundColor = "#8C003B";
  leftSideBar.style.backgroundColor = "#8C003B";
  titleElements.style.backgroundColor = "whitesmoke";
  weatherArrow.style.fill = "#8C003B";
  weatherPopupBody.style.backgroundColor = "#8C003B";
  wordmark.src = "./src/assets/uwWordmarks/UnivWaWordmarkPurple.png";
  title.style.color = "#4b2e83";
  logo.src = "./src/assets/logos/HuskyNavLogoWhite.png";
...
function scrollToLocation(prefix, els, holder) {
  ...
  for (var i = 0; i < els.length; i++) {
    var currentPrefix = els[i].innerHTML
...
function updateLocationOpacities() {
  for (var i = 0; i < locationElements.length; i++) {
    if (
      isElementCentered(locationElements[i], busStopContainer) ||
      isElementCentered(locationElements[i], buildingContainer)
    ) {
...
function navPossible(from, to) {
  return (
    from != "Select Starting Point" && to != "Select Destination" && to != from
  );
}
...
function nav() {
  setNavView(
    locationsMap[fromElement.innerHTML],
    locationsMap[toElement.innerHTML]
  );
  var GETurl =
    "https://huskynavigationserver2.azurewebsites.net/api/pathfind?start=" +
    fromElement.innerHTML.replace(/\s/g, "") + // Remove space from start.
    "&end=" +
    toElement.innerHTML.replace(/\s/g, ""); // Remove space from end.
...
function weatherBalloon() {
  fetch(
    "https://api.openweathermap.org/data/2.5/weather?q=Seattle&appid=0eabe4f0d958928ef5fbeb6346eade3c"
  )
    .then((res) => res.json())
    .then((res) => {
...
function updateData(tempF, tempC, wind, weather, humidity, iconcode) {
  // Updates the icon.
  weatherIcon.src = "./src/assets/openWeatherIcons/" + iconcode + ".png";
  ...
  if (humidity > 75 && weather != "Clear") {
    weatherIconMini.src = "./src/assets/umbrellaIcons/RainIcon.png";
  } else {
    weatherIconMini.src = "./src/assets/umbrellaIcons/NoRainIcon.png";
    weatherIconMini.style.width = "35px";
    weatherIconMini.style.height = "35px";
  }
  // Updates the popup data
  weatherPopupData.innerHTML =
    tempF +
    "&#176F / " +
    tempC +
    "&#176C <br> " +
    wind +
    " <small>MPH</small> Wind";
}
...
function goToPosition(position) {
  ...
  var inXBounds =
    coords[0] > 47.656183871790766 && coords[0] < 47.66121658241639;
  var inYBounds =
    coords[0] > -122.30475545604999 && coords[0] < -122.30798213509433;
  if (!inXBounds || !inYBounds) {
    // Gives map time to reach the current location of the user.
    window.setTimeout(() => {
      alert(
        "You appear to be outside the University of Washington campus. Please note that our service works best for locations near the campus."
      );
    }, 500);
  }
}
...
function populateNavOptions(locGroups, selection) {
  ...
  locGroups.forEach((group, i) => {
    ...
    for (const buildingName in group) {
      if (
        labelMap[i] == "Bus Stops" ||
        disabledLibraries.includes(buildingName)
      ) {
        selection.innerHTML +=
          '<option value="' +
          buildingName +
  ...
  if (document.documentElement.clientWidth <= 600) {
    if (cmd == "off") {
      leftSideBar.style.height =
        document.getElementById("titleElements").offsetHeight + 10 + "px";
...
function handleMapClick(ev) {
  ...
  switch (numCircles) {
    ...
    case 2:
      if (
        nearest.index == destSelection.selectedIndex ||
        nearest.index == startSelection.selectedIndex
      ) {
        // alert("Invalid. Try again.");
        break;
      }
      ev.target
        ? updateDropdown(startSelection, destSelection.selectedIndex, true)
        : null;
      startCircle.remove();
...
function findNearestLoc(latlng) {
  var nearest = {
    index: 0,
    name: "Red Square",
    latlng: navigableLocations["Planetarium"],
  };
  for (const loc in navigableLocations) {
    ...
    if (dist(buildingLatlng, latlng) < dist(nearest.latlng, latlng)) {
      const buildingIndex = startSelection.innerHTML
        .split("n>")
        .findIndex((opt) => opt.includes(loc));
...
function parseNodes(nodesTxt) {
  ...
  for (i = 1; i <= nodeCount; i++) {
    ...
    var isBuilding = !(
      ((name.charAt(0) == "R" || name.charAt(0) == "N") &&
